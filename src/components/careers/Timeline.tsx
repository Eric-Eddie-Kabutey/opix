...
export function ApplicationProcessTimeline({ steps }: { steps: Step[] }) {
  return (
    <ol className="grid gap-5 md:grid-cols-5">
      {steps.map((step, i) => (
        <li key={step.number} className="relative rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex items-center gap-2">
            <span className="font-display text-2xl text-teal-600">{step.number}</span>
            {i < steps.length - 1 && (
              <span className="hidden h-px flex-1 bg-gradient-to-r from-teal-500/40 to-transparent md:block" aria-hidden />
            )}
          </div>
          ...
          {step.duration && (
            <p className="mt-3 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500">
              {step.duration}
            </p>
          )}
        </li>
